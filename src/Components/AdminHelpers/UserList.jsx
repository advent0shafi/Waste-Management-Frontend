...
const TABLE_HEAD = ["User", "Function", "Status", "Employed", "verify", "Make Worker"];
...
export function UserList() {
  ...
  const toggleWorkerStatus = async (id, isWorker) => {
    try {
      await axios.put(`http://127.0.0.1:8000/auth/users/${id}/`, { is_worker: !isWorker });
      fetchUsers();
    } catch (error) {
      console.error("Error updating user:", error);
    }
  ...
  const filteredUsers = users.filter((user) =>
    user.username ? user.username.toLowerCase().includes(search.toLowerCase()) : false
  );
                  ...
                  <tr key={id}>
                    ...
                    <td className={classes}>
                      <Button
                        variant="text"
                        size="sm"
                        color={is_worker ? "red" : "green"}
                        onClick={() => toggleWorkerStatus(id, is_worker)}
                      >
                        {is_worker ? "Remove Worker" : "Set as Worker"}
                      </Button>
                    </td>
                  </tr>
                );
